...
const Librarypage = () => {
  ...
  return (
    <>
      ...
      <div className="lp-arrow-box">
        {currentIndex === 0 ? (
          <IoArrowUndoSharp className="lp-arrow-left end" />
        ) : (
          <IoArrowUndoSharp className="lp-arrow-left" onClick={moveLeft} />
        )}
        <div className="lp-arrow-text">
          <h3>{images[currentIndex+1].title}</h3>
          <p>{images[currentIndex+1].author}</p>
        </div>

        {currentIndex + 3 === images.length ? (
          <IoArrowRedoSharp className="lp-arrow-right end" />
        ) : (
          <IoArrowRedoSharp className="lp-arrow-right" onClick={moveRight} />
        )}
      </div>
    </>
  );
